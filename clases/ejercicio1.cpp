...
void mostrarAgenda(Contacto agenda[], int n){
    ...
    if(n == 0){
        ...
        return; // Corregido: return sin valor en función void
    }
...
void editarContacto(Contacto agenda[], int n){
    ...
    if(pos == -1){
        cout<<"No se ha encontrado el contacto a editar"<<endl; // Corregido: << en vez de <
        return;
    }

    cout<<"Editando contacto: "<<agenda[pos].nombre<<endl; // Corregido: "Editanto" → "Editando"
    cout << "Nuevo telefono: ";
    getline(cin, agenda[pos].telefono);
}

int eliminarContacto(Contacto agenda[], int n){ // Corregido: función debe retornar int
    string nombre;
    ...
    for(int i = pos; i < n - 1; i++){
        agenda[i] = agenda[i+1]; // Corregido: i+1 en vez de i-1
    }
...
void ejecutarAgenda(){
    ...
    do{
        mostrarMenu();
        cin>>opcion;
        cin.ignore(); // Corregido: limpiar buffer antes de getline
